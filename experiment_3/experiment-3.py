...
if __name__=="__main__":
    ...
    parser.add_argument('--model','-mod',default="imEro",help="choose the manipulation, imSkeleton, \
        imskeleton_dis, imClipping")
    ...
    plt.subplot(1,2,2)
    if str(orders.model)=="imSkeleton":
    ...
    if str(orders.model)=="imskeleton_dis":    
        
        f = imskeleton_dis(f,a=45,b=60)
        plt.imshow((f).astype(np.uint8),cmap='gray')
        plt.show()
        
    if str(orders.model)=="imClipping":     
        ...
        plt.imshow((f1-f).astype(np.uint8),cmap='gray')
        plt.show()